...
def main(new_x, name, lof_predictions, lof_scores, ratio, max_length, threshold, method='MDT'):
    ...
    if method == 'MDT':
        ...
        C = CorelsClassifier(max_card=2, c=0.8, n_iter=1, verbosity=["loud", "samples"])

        new_x = np.array(new_x)
        # Fit, and score the model on the training set
        C.fit(new_x, lof_predictions, prediction_name="Has a dirty computer")
...
def number_to_binary_vector(parts, number, value_ranges):
    # 初始化长度为10的全零向量
    binary_vector = np.zeros(parts, dtype=int)

    # 将数字映射到对应的取值范围，并设置相应的位置为1
...
def find_max_min_values(matrix):
    # 将二维数组转置，以便按列处理
...
def split_into_ten_ranges(min_value, max_value, k):
    # 计算每份的间隔大小
    interval_size = (max_value - min_value) / k

    # 生成十等份的值
    ranges = [(min_value + i * interval_size, min_value + (i + 1)* interval_size) for i in range(k)]
...
if __name__ == '__main__':
    ...
    for name in names:
        ...
        ripper_clf.fit(X, lof_predictions) # Or pass X and y data to .fit

        print(ripper_clf.out_model())
        print(len(ripper_clf.ruleset_.rules))

        # print(len(ripper_clf.out_model()))
        from sklearn.metrics import precision_score, recall_score, f1_score
        precision = ripper_clf.score(X, lof_predictions, precision_score)
        recall = ripper_clf.score(X, lof_predictions, recall_score)
        f1 = ripper_clf.score(X, lof_predictions, f1_score)
