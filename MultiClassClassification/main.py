...
def cal_accuracy(stats_list):
    all_statistics = [[stat.num * stat.acc, stat.num] for stat in stats_list]
    return np.sum(np.array(all_statistics)[:, 0]) / np.sum(np.array(all_statistics)[:, 1])


def cal_metric(multi_class, leaf_list, rules_to_expand, r_i):
    if multi_class:
        metric = cal_accuracy(leaf_list + rules_to_expand[r_i:])
    else:
        metric = cal_f1score(leaf_list + rules_to_expand[r_i:])

    return metric

def entropy(rule, lof_predictions):
    # p = rule.acc
    # ent = - p * np.log(p + 1e-8) - (1 - p) * np.log(1 - p + 1e-8)
    # return 1 - ent
    unique_labels, counts = np.unique(lof_predictions[rule.idxes], return_counts=True)
    counts = counts / np.sum(counts)
    ent = - np.sum(counts * np.log(counts + 1e-8))
    return 1 - ent


def cal_list_entropy(stats_list, lof_predictions, return_detail=False):
    if len(stats_list) == 0:
        if return_detail:
            return tuple([0] * 2)
        else:
    ...
    entropy_list = np.array([1 - entropy(rule, lof_predictions) for rule in stats_list])
...
def cal_split_entropy(X, y, cur_rule, col, val):
    ...
    # find the label for two new branches
    unique_labels, count = np.unique(tmp_y_1, return_counts=True)
    count = count / np.sum(count)
    ent_1 = - np.sum(count * np.log(count + 1e-8))


    unique_labels, count = np.unique(tmp_y_2, return_counts=True)
    count = count / np.sum(count)
    ent_2 = - np.sum(count * np.log(count + 1e-8))
    ...
    new_avg_entropy = len(tmp_y_1) * (1 - ent_1) + len(tmp_y_2) * (1 - ent_2)
...
# def traverse(root, rules, attributes=None, intervals=None, depth=0):
#
#
...
#             cur_intervals = []
#         else:
#             cur_intervals = copy.deepcopy(intervals)
#         if attributes is None:
#             cur_attributes = []
#         else:
#             cur_attributes = copy.deepcopy(attributes)
#
#         assert cur_attributes is not None
#
#         update_attributes_and_intervals(cur_attributes, cur_intervals,
#                 root.rule.split_attribute, root.rule.split_num, mode='left')
#
#         traverse(root.lchild, rules, cur_attributes, cur_intervals, depth=depth+1)
#
...
#
#         update_attributes_and_intervals(cur_attributes, cur_intervals,
#                 root.rule.split_attribute, root.rule.split_num, mode='right')
#         traverse(root.rchild, rules, cur_attributes, cur_intervals, depth=depth+1)
#
#     if root.lchild is None and root.rchild is None:
#         rules.append(root.rule)
#         root.rule.attributes = attributes
#         root.rule.intervals = intervals
#         assert set(rules[-1].attributes) == set(attributes)
...
def cal_length_entropy_gain(rule, columns, X, y, max_length=15):
    ...
    for col in columns:
        ...
        for val in np.sort(np.unique(X[rule.idxes, col]))[:-1]:
            ...
            # idxes_1 = rule.idxes[np.where(X[rule.idxes, col] <= val)]
            # idxes_2 = rule.idxes[np.where(X[rule.idxes, col] > val)]
            ...
            entropy_gain = new_avg_entropy - entropy(rule, y) * rule.num
...
def expand_rule(X, rule_to_expand, predictions, leaf_list, stats_list, stats, print_result=True):
    # find the rule in stats_list, need to optimize
    # for r in stats_list:
    #     if r == rule_to_expand:
    ...
    unique_labels, counts = np.unique(predictions[idxes_smaller], return_counts=True)
    label_1 = unique_labels[np.argmax(counts)]

    unique_labels, counts = np.unique(predictions[idxes_larger], return_counts=True)
    label_2 = unique_labels[np.argmax(counts)]
...
def expand_rule_and_add_to_list(leaf_list, stats_list, stats, rule, cur_C, columns, predictions,
    ...
    for rule, rule_idx in zip(new_rules, new_idxes):
        ...
        if not rule.length_divide_entropy == -1:
            ...
            if rule_C < cur_C:
                avg_entropy0, total_length0 = cal_list_entropy(leaf_list, lof_predictions, return_detail=True)
                # f1_score_1 = cal_f1score(leaf_list)
                # if f1_score_1 > 0.88: return
                # old_leaf_list = copy.deepcopy(leaf_list)
                ...
                # if not old_leaf_list[-(2-i)].num == leaf_list[-1].num + leaf_list[-2].num:
                #     print("wrong")

                f1_score_2 = cal_f1score(leaf_list)
                # if not f1_score_2 >= f1_score_1:
                #     print("wrong")
...
def buildRuleTree(X, predictions, stats, cur_rule, stats_list, leaf_list, rows, columns, max_length, threshold,
                  print_result=True, multi_class=False):
    '''
    X: Data Points
    ...
    if multi_class:
        metric = cal_accuracy(leaf_list)
    else:
        metric = cal_f1score(leaf_list)

    while metric < threshold:

        avg_entropy0, total_length0 = cal_list_entropy(leaf_list, predictions, return_detail=True)
        ...
        for idx, rule in enumerate(leaf_list):
            ...
            if rule.length_divide_entropy == -1:
                ...
                leaf_C.append(rule.length_divide_entropy * avg_entropy0 * total_length0 - sum(
                    [len(r.attributes) for r in leaf_list]))
            # if rule.length_divide_entropy * avg_entropy0 * total_length0 - sum([len(r.attributes) for r in leaf_list]) < cur_C:
            #     cur_C = rule.length_divide_entropy * avg_entropy0 * total_length0 - sum([len(r.attributes) for r in leaf_list])
            #     cur_idx = idx
        ...
        if len(idxes) > 0:
            ...
            for r_i, r in enumerate(rules_to_expand):
                ...
                # expand_rule_and_add_to_list(leaf_list, stats_list, stats, r, cur_C, columns, predictions,
                #                             max_length, avg_entropy0, total_length0, print_result=True)
                metric = cal_metric(multi_class, leaf_list, rules_to_expand, r_i)
        ...
        if print_result:
            ...
            print("C = ", cur_C, "The Metric:", metric)

    if print_result:
        if multi_class:
            print("Final Accuracy:", cal_accuracy(leaf_list))
        else:
    ...
    # draw_C(rule_num_along_the_process, C_along_the_process, name)
...
def calculate(X, predictions, criterion=1, max_length=10, max_depth=-1, threshold=0.90, print_result=True,
              return_rule_num=False, multi_class=False):
    ...
    root_rule = stats(len(X), np.sum(predictions == label)/len(predictions), label, rows, [])
    root = Node()
    ...
    leaf_list = buildRuleTree(X, predictions, stats, root_rule, stats_list, leaf_list, rows, columns, max_length,
                              threshold, print_result=print_result, multi_class=multi_class)
    # root = buildTree(X, predictions, stats, root_rule, stats_list, rows, columns)
...
def main(name, lof_predictions, lof_scores, ratio, max_length, threshold, method='MDT'):
    ...
    multi_class = len(np.unique(lof_predictions)) > 2
    ...
    if method == 'MDT':
        ...
        # root, length, num = calculate(X_new, lof_predictions_new, max_length=max_length, criterion=1, print_result=True,
        root, length, num = calculate(X, lof_predictions, max_length=max_length, criterion=1, print_result=True,
                                      threshold=threshold, return_rule_num=True, multi_class=multi_class)
        end_time = time.time()

        predictions = root.predict(X)

        if multi_class:
            metric = metrics.accuracy_score(lof_predictions, predictions)

        else:
            metric = metrics.f1_score(lof_predictions, predictions)

        time = end_time - start_time
        print("Final Metric:", metric)
        print("Time consumed:", time)
        return metric, time, length, num
        ...
        for ccp_alpha in ccp_alphas[::-1]:
            ...
            if multi_class:
                metric = metrics.accuracy_score(lof_predictions, clf.predict(X))
            else:
                metric =  metrics.f1_score(lof_predictions, clf.predict(X))
        ...
        return metric, 0, np.sum(lengths), len(lengths)
        ...
        for depth in range(1, 25):
            ...
            if multi_class:
                metric = metrics.accuracy_score(lof_predictions, clf.predict(X))
                print(f"Depth: {depth}, Accuracy: {metric}")
                if metric > threshold: break

            else:
                metric = metrics.f1_score(lof_predictions, clf.predict(X))
                print(f"Depth: {depth}, F1-score: {metric}")
                if metric > threshold: break
...
if __name__ == '__main__':
    ...
    for name in names:
        ...
        if name not in ['MNIST', 'Wine']:
            ...
            # X, y = load_data(name, data_dir=data_dir)
            X, y = load_data(data_dir=data_dir)


            # predictions and scores are generated from logistic regression
            # clf = LogisticRegression(random_state=0, max_iter=5000, solver='liblinear').fit(X, y)
            # lof_predictions = clf.predict(X)
            # lof_scores = clf.predict_proba(X)[:, 1]
            ...
            for ratio in [1.0]:
                for max_length in max_lengths:
                    for threshold in thresholds:
                        ...
                        nums = []
                        for i in range(1):
                            result = main(name, lof_predictions, lof_scores, ratio, max_length, threshold, method)
                        ...
                        df = pd.DataFrame(results, columns=['name', 'ratio', 'max_length', 'F1_score/Acc', 'time', 'lengths', 'nums'])
